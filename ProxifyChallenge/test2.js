...
function solution(A) {
  ...
  for (let i = 0; i < A.length; i += 1) {
    const firstSubString = A[i]

    const sequentialStrings = A.slice(1, A.length - 1)

    console.log({ sequentialStrings })

    for (let j = i + 1; j < A.length; j += 1) {
      const secondSubString = A[j]

      if (hasAtLeastOneSimilarLetter(firstSubString, secondSubString)) {
        continue
      }

      const currentConcatenationLength = firstSubString.length + secondSubString.length

      if (currentConcatenationLength > biggestSoFar) {
        biggestSoFar = currentConcatenationLength
      }
    }
  ...
  function hasAtLeastOneSimilarLetter(stringOne, stringTwo) {
    const completeString = stringTwo ? [...stringOne, ...stringTwo] : [...stringOne]

    while (completeString.length > 0) {
      const letter = completeString.splice(0, 1)[0]

      const isDuplicate = completeString.indexOf(letter) >= 0
